...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="md:Extensions/mdattr:EntityAttributes">
        ...
        <xsl:if test="count(saml:Attribute[@Name='urn:x-safire.ac.za:quirks'])>1">
            <xsl:call-template name="error">
                <xsl:with-param name="m">More than one urn:x-safire.ac.za:quirks EntityAttribute</xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    ...
    <xsl:template match="md:EntityDescriptor[md:SPSSODescriptor]">
        <xsl:if test="md:Extensions/mdattr:EntityAttributes/saml:Attribute[@Name='urn:x-safire.ac.za:quirks']">
            <xsl:call-template name="error">
                <xsl:with-param name="m">urn:x-safire.ac.za:quirks should not be set for Service Providers</xsl:with-param>
            </xsl:call-template>
        </xsl:if>
